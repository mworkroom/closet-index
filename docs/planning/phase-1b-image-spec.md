...
예외적으로 `Acc-Waist`는 평평하게 펼친 누끼와 허리에 감아 묶은 착용 형태의 비율 차이가 크므로, 해당 카테고리에만 600×260 표시 상자를 적용한다. 원본 파일을 수정하지 않고 합성 단계에서만 허리에 감긴 체감 비율로 표시한다.
...
`Acc-Waist`는 일반 비례 배율 대신 900×1200 canvas에서 600×260 표시 상자를 사용하고 z-index 55로 상의·하의보다 앞, 아우터·목 액세서리·가방보다 뒤에 둔다.
