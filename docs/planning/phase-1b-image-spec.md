# Closet Index Phase 1B Image Preparation & Normalization Spec

- 작성일: 2026-07-27
- 상태: 입력 규칙·composition v1 원격 운영·composition v3 종횡비 정규화와 실시간 목록 합성 구현, ready cutout 19개와 Outfit preview v1 원격 운영
- 관련 문서: [Phase 1B Plan](./phase-1b-plan.md), [Pilot Outfits](./phase-1b-pilot-outfits.md)

## 1. 목표

J가 Item마다 직접 확대·축소·중앙 정렬하지 않아도 같은 누끼 하나를 여러 Outfit에서 일관되게 재사용할 수 있게 한다.

핵심 원칙:

- J는 배경이 투명하고 Item 전체가 보이는 누끼만 준비한다.
- 투명 여백 제거, 중심 계산, 카테고리별 크기 조정은 준비 도구가 자동으로 처리한다.
- composition v3는 620×760 기준 템플릿에 카테고리별 최대 표시 높이를 함께 적용해, 같은 카테고리의 가로형·세로형 누끼가 비슷한 시각 척도로 보이게 한다.
- 양말·신발·가방처럼 실제 크기와 카드에서 보여줄 크기가 다른 항목은 카테고리 기본 배율로 처리한다.
- 자동 배치로 해결되지 않는 소수 Item만 개별 `scale`, `position_x`, `position_y` override를 사용한다.
- 원본 누끼는 수정하지 않고, 정규화와 합성 결과를 언제든 다시 만들 수 있게 한다.

## 2. J가 준비할 이미지

권장 입력:

- 투명 배경 PNG 또는 WebP
- Item 하나가 완전히 보이는 이미지
- 세로·가로 방향은 실제로 앱에서 보이길 원하는 방향
- 가능하면 긴 변 1200px 이상
- 가능하면 sRGB 색상

입력 출처는 하나로 통일하지 않는다.

- iPhone으로 직접 촬영한 원본에서 만든 누끼
- 과거 옷장 앱의 Item 상세 화면을 캡처해 만든 누끼
- 쇼핑몰 상품 사진에서 만든 누끼
- 이미 별도로 보관 중인 이미지에서 만든 누끼

출처나 원본 해상도보다 최종 화면에서 Item을 알아볼 수 있는지가 우선이다. 긴 변 1200px 이상은 재가공 여유를 위한 권장값이지 업로드 필수 조건이 아니다. 긴 변이 500~1200px인 캡처 기반 이미지도 Item 전체와 주요 색·형태를 알아볼 수 있으면 허용하며, 작은 입력을 억지로 확대하지 않는다.

J가 하지 않아도 되는 작업:

- 정사각형 또는 특정 캔버스 크기에 맞추기
- 투명 여백을 정밀하게 자르기
- Item을 캔버스 중앙에 놓기
- 카테고리에 따라 직접 축소하기
- iPhone 앱에서 보던 크기에 맞추기
- WebP로 최적화하기

피해야 할 입력:

- Item 일부가 잘린 이미지
- 불투명한 흰색 배경
- 여러 색상의 다른 Item이 한 파일에 섞인 이미지
- 아주 작은 이미지를 억지로 확대해 흐려진 파일
- 누끼 주변에 큰 그림자나 장식이 포함된 이미지

신발·양말처럼 한 쌍인 물건은 J가 최종 카드에서 보고 싶은 모습 그대로 한 파일에 넣는다. 한 짝만 보이길 원하면 한 짝, 한 쌍이 보이길 원하면 한 쌍을 하나의 Item 이미지로 준비한다.

### 2.1 충분한 화질의 판정 기준

Phase 1B 이미지의 목적은 원본 사진을 감상하거나 확대 보존하는 것이 아니라 Closet과 Outfit에서 어떤 Item인지 빠르게 알아보는 것이다.

다음 조건을 만족하면 충분한 화질로 본다.

- iPhone의 Closet 목록과 Item 상세에서 Item 종류·색·대략적인 형태를 구분할 수 있다.
- Outfit preview에서 다른 Item과 혼동하지 않는다.
- 압축 흔적이 조금 보여도 선택과 착용 기록에 방해가 되지 않는다.
- Retired Item은 이미지가 없어도 되며, 필요할 때만 과거 앱 캡처를 사용한다.

고해상도 자체는 합격 기준이 아니다. 같은 Item을 알아보는 데 차이가 없다면 더 작은 파일을 선택한다.

## 3. 자동 정규화 과정

```text
J의 투명 누끼
→ alpha 영역으로 실제 Item 경계 계산
→ 불필요한 투명 여백 제거
→ 2~3% 안전 여백 추가
→ 620×760 기준 템플릿에 카테고리별 상대 배율 적용
→ 카테고리별 slot 안에 비율 유지 fit
→ 카테고리별 anchor에 정렬
→ 필요한 경우 Item별 override 적용
→ Outfit preview 합성
```

### 3.1 투명 여백 제거

- 이미지 전체 크기가 아니라 불투명·반투명 픽셀이 존재하는 실제 경계 상자를 사용한다.
- 누끼 앱이 만든 캔버스 크기나 중심 위치가 파일마다 달라도 결과에 영향을 주지 않는다.
- 가장자리 털, 니트 보풀, 반투명 그림자가 잘리지 않도록 낮은 alpha threshold와 작은 안전 여백을 둔다.

### 3.2 비율 유지

각 Item은 다음 계산으로 slot 안에 맞춘다.

```text
scale = min(slot 최대 너비 / Item 실제 너비,
            slot 최대 높이 / Item 실제 높이)
```

- 기본적으로 가로세로 비율을 바꾸지 않는다.
- 기본적으로 slot을 채우기 위해 늘여 찌그러뜨리지 않는다.
- 작은 원본을 화질이 깨질 정도로 확대하지 않는다.

`Acc-Waist`도 다른 카테고리와 마찬가지로 원본 누끼의 가로세로 비율을 유지한다. 합성 단계에서 임의로 늘이거나 누르지 않으며, 표시 크기와 위치는 비율이 보존된 결과를 기준으로 조정한다.

### 3.3 카테고리별 anchor

단순한 이미지 중앙점 하나만 사용하지 않는다.

| 카테고리 | 정렬 기준 |
|---|---|
| 목 액세서리 | 실제 경계의 가로 중앙과 위쪽 기준 |
| 아우터 | 가로 중앙과 위쪽 기준 |
| 상의 | 가로 중앙과 위쪽 기준 |
| 허리 액세서리 | 허리 중앙과 위쪽 기준, 상의·하의 위 레이어 |
| 하의·원피스 | 가로 중앙과 위쪽 기준 |
| 양말 | 가로 중앙과 아래 기준, 가장 낮은 z-index |
| 신발 | 가로 중앙과 바닥선 기준 |
| 가방 | 실제 경계의 중앙 |

신발과 양말은 중심점보다 바닥선을 맞추는 편이 서로 다른 높이의 이미지에서도 안정적이다.

## 4. 카테고리 기본 크기

정확한 좌표는 Batch 0 누끼 7개로 확정하지만 크기 관계는 다음과 같이 고정한다.

| 종류 | 상대 크기 |
|---|---|
| 아우터 | 가장 큼 |
| 하의·원피스 | 큼 |
| 오른쪽 상의 | 중간 |
| 허리 액세서리 | 중간·넓음 |
| 가방 | 중간 |
| 신발 | 작음 |
| 목 액세서리 | 작음 |
| 양말 | 가장 작음 |

900 × 1200 preview 기준 초기 최대 영역:

| Slot | 최대 너비 | 최대 높이 |
|---|---:|---:|
| main-neck | 38% | 16% |
| main-outer | 48% | 38% |
| main-waist | 67% | 원본 비율로 자동 계산 |
| main-bottom | 42% | 38% |
| main-socks | 18% | 13% |
| main-shoes | 30% | 16% |
| side-top | 30% | 28% |
| side-bag | 30% | 28% |

이 값은 J가 누끼를 만들 때 맞출 규격이 아니다. 합성 도구의 자동 배율 초기값이며 첫 시안에서만 조정한다.

### 4.1 composition v2 기준 템플릿 프로토타입

J가 만든 `lookbook1.png`, `lookbook2.png`, `lookbook3.png`의 체감 비율을 기준으로, 원본 경계 상자를 각 slot에 최대한 채우는 v1 방식과 별도로 공통 기준 템플릿을 도입한다. 세 기준 이미지는 로컬 시각 참고 자료이며 Git이나 Supabase에 저장하지 않는다.

기준 템플릿은 620×760이고, 실제 렌더링 최대 크기는 이 템플릿에 다음 배율을 곱한 뒤 Item 비율을 유지해 계산한다.

| 종류 | v2 기준 배율 |
|---|---:|
| 앞·뒤 아우터 | 100% |
| 원피스 | 95% |
| 아우터가 없는 일반 상의 | 85% |
| 하의 | 64% |
| 아우터와 함께 우측에 놓는 `Top-T-shirts` | 45% |
| `Top-T-shirts-innerwear` | 40% |
| 가방 | 34% |
| 신발 | 28% |
| 목 액세서리 | 27% |
| 양말 | 20% |

`Acc-Waist`는 900×1200 canvas에서 최대 너비 600px을 기준으로 원본 비율을 유지해 크기를 계산하고, z-index 55로 상의·하의보다 앞, 아우터·목 액세서리·가방보다 뒤에 둔다.

`visualScale`은 카테고리의 공통 기준이고, 기존 `scale`은 같은 카테고리에서도 크기가 특별한 Item에만 곱하는 예외값으로 유지한다. Outfit마다 같은 Item 크기를 다시 맞추지 않는다.

### 4.2 composition v3 종횡비 정규화와 하단 간격

Batch 2에서 가로로 넓은 Batch 0 가디건과 세로형 후드 재킷·티셔츠에 같은 620×760 비례 상자를 적용하면, 세로형 Item의 실제 표시 높이가 지나치게 커지는 문제가 확인됐다. composition v3는 기존 `visualScale`을 유지하면서 다음 최대 표시 높이를 추가한다.

- 아우터: 최대 500px
- 아우터가 없는 일반 상의: 최대 440px

브라우저와 로컬 Sharp 합성 도구는 cutout metadata의 실제 너비·높이를 사용해 카테고리 상자 안에 비율을 유지하며 맞춘다. 따라서 Batch 0 가디건처럼 가로로 넓은 Item은 승인된 기존 크기를 유지하고, 세로형 후드 재킷과 일반 상의만 높이 상한에 맞춰 함께 축소된다.

하의·원피스의 끝과 신발 시작점에는 900×1200 canvas 기준 32px 기본 간격을 둔다. 기본 간격을 먼저 계산한 뒤 Outfit별 `position_x`·`position_y`를 적용하므로, 자동 배치 후 4px 화살표 미세 이동은 계속 유효하다. 양말이 있으면 이 구간에 양말이 보일 수 있지만 하의와 신발의 기본 경계는 동일하게 유지한다.

### 4.3 composition v4와 J 배치 기본값

composition v4에서는 iPhone 실사용 확인을 반영해 신발 category 기준 `visualScale`을 26.6%로 유지하면서 placement 기본 크기를 80%로 표시한다. 하의·원피스와 신발 사이 기준 간격은 16px로 조정하고 가방 slot은 주열 쪽으로 40px 옮겼다.

이후 Supabase에 저장된 Outfit·Item 배치를 읽기 전용으로 반복 집계한 결과, 상의는 52~56px 아래로 옮기고 신발은 80%로 줄이는 수정 방향이 추가로 반복됐다.

| 종류 | 미수정 Item에 적용할 기본값 |
|---|---:|
| 아우터 | 크기 90%, 아래 52px |
| 아우터 없이 주열에 놓는 일반 상의 | 크기 90%, 아래 52px |
| 신발 | 크기 80%, 위 20px |
| 가방 | 크기 100%, 왼쪽 80px |
| 스카프·목 액세서리 | 아래 144px |
| 양말 | 크기 150% |
| 허리 액세서리 | 크기 70%, 왼쪽 100px |

이 값은 `visualScale`이나 개별 저장값을 일괄 덮어쓰지 않는 placement 기본값이다. Supabase의 `position_x`, `position_y`, `scale`이 `NULL`인 미수정 Item에만 사용하며, 숫자가 저장된 Item은 `0`, `1`을 포함해 기존 명시값을 우선한다. 따라서 이미 J가 수정한 Outfit은 현재 모습을 유지하고, 아직 수정하지 않은 Outfit만 반복된 선호를 초기 배치로 받는다.

신발 크기를 바꿀 때 하의가 함께 움직이지 않도록 하의·신발 간격 계산은 신발의 80% 기본 크기를 기준으로 고정한다. 사용자가 신발을 확대·축소해도 신발 layer만 달라지고 하의의 위치와 크기는 유지된다.

아우터와 함께 오른쪽 보조열에 놓이는 `Top-T-shirts`, `Top-T-shirts-innerwear`, 하의는 현재 표본이 부족하거나 수정 방향이 달라 공통 placement 기본값을 추가하지 않는다.

## 5. 예외 처리

자동 규칙으로 대부분을 처리하고 다음 경우에만 Item별 override를 둔다.

- 롱코트처럼 같은 Outer에서도 유난히 긴 Item
- 작은 숄과 큰 머플러처럼 크기 차이가 큰 목 액세서리
- 니삭스와 짧은 양말처럼 형태가 크게 다른 Socks
- 토트백과 작은 크로스백의 시각적 크기 차이
- 한 파일 안의 신발 두 짝 간격이 지나치게 넓은 경우
- 누끼 자체의 비대칭 때문에 기하학적 중앙이 어색한 경우

override:

```text
scale       기본 크기에 곱할 보정값
position_x  좌우 이동
position_y  상하 이동
z_index     레이어 순서
```

보정값은 원본 이미지에 적용하지 않고 `closet_outfit_items` 또는 별도 manifest에 기록한다. 같은 Item을 여러 Outfit에서 항상 같은 방식으로 보정할지, 특정 Outfit에서만 보정할지는 Batch 0 결과로 결정한다.

## 6. Batch 0 검증

첫 Favorite Outfit의 7개 Item으로 다음을 확인한다.

- 목 액세서리와 아우터의 간격
- 아우터와 하의의 상대 크기
- 일반 상의는 오른쪽 보조열에, 이너웨어는 아우터 아래 레이어에 배치되는지
- 이너웨어가 아우터와 하의 사이에서 필요한 만큼만 보이는지
- 양말이 자동으로 충분히 작아지는지
- 양말이 최하단 레이어, 신발이 그 앞에 보이는지
- 오른쪽 가방과 왼쪽 주열의 간격이 지나치게 넓지 않은지
- 전체 Outfit이 3열 Lookbook thumbnail에서도 식별되는지

Batch 0에서 slot 기본값을 한 번 확정한 뒤 Batch 1 누끼 12개를 합성한다. Batch 1에서 반복적으로 같은 보정이 필요하면 개별 Item 문제가 아니라 카테고리 기본값 문제로 보고 slot 설정을 수정한다.

2026-07-28 Batch 1 12개(J 전달명 Batch 2)를 업로드한 뒤, ready cutout이 모두 있는 Outfit은 저장된 preview보다 현재 composition 실시간 합성을 우선하도록 했다. 이로써 상세 화면에서 저장한 `position_x`·`position_y` 보정이 HOME·LOOKBOOK·FAVORITE thumbnail에도 같은 규칙으로 반영된다. cutout이 불완전한 Outfit은 기존 저장 preview 또는 스와치 fallback을 그대로 사용한다.

## 7. Batch 0 레이어와 미세 위치 조정

J의 3개 기준 시안과 정정 내용을 반영해 composition v2에서 가장 위에 보이는 레이어부터 다음 순서로 고정한다.

```text
가방
목 액세서리
앞쪽 아우터
일반 상의·뒤쪽 아우터
Top-T-shirts-innerwear
하의·원피스
신발
양말
```

- `Top-T-shirts-innerwear`는 왼쪽 `main-innerwear`에 배치하고, 하의·원피스보다 위이면서 앞·뒤 아우터보다 아래에 둔다.
- 이너웨어는 아우터 뒤에 깔고, 하의가 시작되기 전 좁은 구간에서만 하단이 보이게 한다.
- `Top-T-shirts`는 아우터와 함께 등장할 때만 오른쪽 `side-top`으로 보내고 전체 합성의 최하위 레이어로 내린다. 아우터가 없으면 왼쪽 `main-upper`의 일반 상의 크기와 레이어를 사용한다.
- 다른 일반 상의는 기본적으로 `main-upper`를 사용한다.
- 여러 상체 Item이 앞뒤로 겹치면 `outer-front`, `outer-back`, `main-top` composition role로 같은 `main-upper` slot을 공유한다.
- 원피스는 하의 slot이 아니라 상체부터 하단까지 이어지는 `main-dress`를 사용한다.
- 가방은 오른쪽 보조열을 유지하면서 전체 합성의 최상위 레이어로 둔다.

자동 배치 뒤 예외적인 위치 보정이 필요하면 자유 드래그보다 네 방향 화살표를 사용한다.

- 한 번 누를 때 4px 이동
- 길게 누르면 같은 방향으로 연속 이동
- 이동값은 `position_x`, `position_y`로 저장
- `원위치`로 해당 Item의 이동값만 0으로 복원
- 크기 조정은 기본 화면에 두지 않고 실제 예외가 확인될 때만 별도 override로 제공

## 8. 파일 용량과 원격 저장 정책

Supabase에는 앱 운영에 필요한 최적화 결과만 기본 저장한다.

| 종류 | 원격 저장 | 기준 |
|---|---|---|
| 입력 원본·고해상도 PNG | 기본 저장 안 함 | 누끼 검증과 재처리 동안 로컬에만 임시 보관 |
| Item 누끼 | 저장 | 투명 WebP, 긴 변 최대 1600px에서 시작해 목표 용량까지 적응형 압축 |
| Outfit preview | 저장 | 투명 WebP, 900 × 1200px 고정, quality 90에서 시작해 적응형 압축 |

용량 목표:

- Item 누끼는 500KB 이하를 목표로 한다. 1600px·quality 90부터 시작해 품질과 긴 변을 순서대로 낮추며, 최종 결과가 700KB를 넘으면 업로드 준비를 차단한다.
- Outfit preview는 900 × 1200px를 유지하면서 quality 90부터 단계적으로 낮춰 350KB 이하로 만든다. 최저 후보에서도 목표를 넘으면 업로드 준비를 차단한다.
- 입력 파일 용량은 합격 기준으로 사용하지 않고 준비 도구가 정규화한 결과를 검사한다.

아이폰 앱 캡처처럼 같은 정사각형 프레임에서 만든 누끼가 100~1000KB로 다양해도 입력 자체는 허용한다. alpha trim과 2.5% 안전 여백 뒤에 위 후보를 적용하므로 사람이 파일마다 해상도나 quality를 미리 맞추지 않는다. 원본 캡처와 로컬 누끼 입력은 원격 asset plan과 metadata에서 제외한다.

Batch 0 실측에서 Item 누끼 7개는 총 약 1.15MB, 평균 약 164KB였고 Outfit preview 1개는 약 132KB였다. 이 평균을 현재 Item 451개와 Outfit 507개 전체에 단순 적용하면 앱 표시용 이미지 예상치는 약 141MB다. 고해상도 원본을 제외하면 Supabase Free Plan의 1GB Storage 안에서 충분한 여유가 있다.

정책 변경 전에 업로드된 Batch 0 original PNG 7개 약 12.78MB는 즉시 삭제하지 않는다. 새 준비·업로드 도구는 cutout만 원격 asset plan과 metadata에 포함하며, 기존 original은 cutout과 preview의 원격 읽기 및 재생성이 확인된 별도 정리 단계에서만 삭제 여부를 결정한다.

## 9. 결론

J는 Item의 실제 크기, 이미지 캔버스 중심, 양말 축소 비율을 맞출 필요가 없다. 배경이 투명하고 Item 전체가 보이는 누끼를 준비하면 나머지는 자동 정규화한다.

예외 보정은 가능하지만, 모든 이미지를 사람이 드래그해 맞추는 흐름을 다시 만들지는 않는다.

이미지 품질은 고해상도 보존이 아니라 Item 인지 가능성으로 판단한다. iPhone 촬영본, 과거 앱 캡처, 쇼핑몰 이미지가 섞여도 최종 Item을 알아볼 수 있으면 같은 준비 과정으로 사용하며, Supabase에는 최적화된 누끼와 Outfit preview만 기본 보관한다.
