...
- 비 적합성은 모든 Item, 장거리 걷기는 신발과 가방을 기본 가능으로 보며 불가 예외만 점진적으로 태깅한다.
...
Item 상세 페이지에서 교체 계보를 확인하고, 구매·착용·관리 사건을 바탕으로 재구매, 장기 미착용 점검·정리 후보, 손세탁·드라이클리닝 필요 시점을 관리한다.

### 주요 범위

- Item 상세의 직접 부모 → 본인 → 직접 자식 Replacement Line
- 재구매 날짜·구매 수량·현재 보유 수량 기록과 최근 구매 기준 교체 계산
- 착용 기록 없음·2년 이상 3년 미만의 `점검`과 3년 이상 미착용의 `정리 후보` 신호
- Category 규칙과 최근 관리 이후 착용 횟수를 사용한 손세탁·드라이클리닝 관리
- Item 목록의 우선순위 배지 1개와 Item 상세의 전체 배지
- More → Maintenance의 점검·정리 후보·교체 Category 토글 목록과 별도 Laundry의 손세탁·드라이클리닝 목록
...
- 운영체제 알림 없이 Maintenance와 Laundry에서 필요한 Item을 목적별 영역으로 확인할 수 있다.
