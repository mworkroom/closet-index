...
자동으로 API를 반복 호출하지 않는다. 날짜·시각·위치를 정한 뒤 `날씨로 추천 보기`를 눌러 명시적으로 조회하며, 같은 발표 예보는 세션 캐시를 재사용한다.
...
`날씨로 추천 보기`를 눌러 조회가 성공하면 출발·귀가 온도와 비 조건을 기존 추천 입력에 바로 반영하고 추천 결과를 표시한다. 사용자가 먼저 누른 한 번의 명시적 동작 안에서 조회와 적용을 완료한다.
...
- HOME에 기본 예보 위치, 날짜, 출발·귀가 정시 선택, 명시적 `날씨로 추천 보기`를 추가했다.
...
- `날씨로 추천 보기`를 한 번 누르면 예보 조회 성공 직후 기존 `RecommendationInput`에 반영하고 추천 결과를 표시한다.
