...
- 상단에 이전 달 화살표, 네이티브 연·월 선택기, 다음 달 화살표를 둔다.
...
- 날짜 행은 92px 고정 높이를 사용한다. 날짜 숫자 영역을 제외한 착장 wrapper는 최대 48px 너비와 3:4 비율을 유지하며, 달력 아래 남는 공간을 억지로 채우지 않는다.
...
- 390×844 브라우저에서 5주·6주 달력, 월 이동, 단일 착장 연결, 복수 착장 선택 시트, console warning·error 부재를 확인했다.
